...
% A.1. Problem
modelname = 'sis100_small';                                                %        : model name
...
Bxy = curl(prb.mesh,az);                                                   % [T]    : magnetic flux density
Bm = pyth(Bxy);                                                            % [T]    : magnitude of the magnetic flux density
if plotflag>=2
...
Wmagn_ownpost = sum(sum(1/2*Bxy.*nuelem.*Bxy,2).*prb.mesh.area.*prb.mesh.depth); % [J]     : magnetic energy
...
Knu = curlcurl_ll(prb.mesh,nuelem);                                        % [1/H]   : FE reluctance matrix (xy-direction)
Wmagn_femm_alt = 1/2*az'*Knu*az;                                           % [J]     : alternative method for calculating the magnetic energy (combining FEMM solution and Matlab matrix)
report('Magnetic energy',Wmagn_femm_alt','J (FEMM+Matlab)');
Pstr = current_Pstr(prb);                                                  % []      : winding-function coupling matrix
...
ubcs = Kbcs\(Pbcs*Iapp);                                                   % [Wb]    : solve for the line-integrated magnetic vector potentials
...
Bxy = curl(prb.mesh,u);                                                    % [T]     : magnetic flux density
Bm = pyth(Bxy);                                                            % [T]    : magnitude of the magnetic flux density
if plotflag>=2
...
Wmagn_ownfe = sum(sum(1/2*Bxy.*nuelem.*Bxy,2).*prb.mesh.area.*prb.mesh.depth); % [J]     : magnetic energy (from own FE solution)
Wmagn_ownfe_alt = 1/2*u'*Knu*u;                                            % [J]     : alternative method for calculating the magnetic energy
...
while (it<maxiter) & (relerrnlin>epsnlin)
  it = it+1;
  ufem_prev = ufem;
  % E.1. Update for the nonlinearity
  B = curl(prb.mesh,ufem);
  [dummy,nu] = nlin_evaluate(nlin,B(idxnlinelem,:));
  nuelem(idxnlinelem,:) = [nu nu];
  % E.2. Compute the system of equations
  Knu = curlcurl_ll(prb.mesh,nuelem);                                      % [1/H]  : FE reluctance matrix (xy-direction)
  Kbcs = bdrycond_shrink(prb.bdrycond,idxdof,Knu);                         %         : shrink the system matrix for the boundary conditions
  % E.3. Solve the system of equations
  ubcs = Kbcs\(Pbcs*Iapp);                                                 % [Wb]   : solve the reduced system of equations
  ufem = bdrycond_inflate(prb.bdrycond,ubcs,numunknown,idxdof);            % [Wb]    : insert the boundary conditions
  ufem = alpha*ufem+(1-alpha)*ufem_prev;                                   % [Wb]   : relaxation of the solution
  % E.4. Check for convergence
  relerrnlin = norm(ufem-ufem_prev)/norm(ufem);                            % []     : relativ nonlinear error
...
while (it<maxiter) & (relerrnlin>epsnlin)
  ...
  B = curl(prb.mesh,ufem);
  Bred = B(idxnlinelem,:);
  [Hred,nured,nudred,dnudB2red] = nlin_evaluate(nlin,Bred);
  nuelem(idxnlinelem,:) = [nured nured];
  dnudB2(idxnlinelem,:) = dnudB2red;
  Hc(idxnlinelem,:) = Hred-nudred*[1 1].*Bred;
  % E.2. Compute the system of equations
  [Knud,jHc] = curlcurl_ll_nonlinear(prb.mesh,B,nuelem,dnudB2,Hc);         % [A/Wb] : reluctance matrix
  ffem = jHc+Pstr*Iapp;                                                    % [A]    : righthandside
  % E.3. Solve the system of equations
  [Kbcs,fbcs] = bdrycond_shrink(prb.bdrycond,idxdof,Knud,ffem);            %         : shrink the system matrix for the boundary conditions
  ubcs = Kbcs\fbcs;                                                        % [Wb]   : solve the reduced system of equations
  ufem = bdrycond_inflate(prb.bdrycond,ubcs,numunknown,idxdof);            % [Wb]    : insert the boundary conditions
  ufem = alpha*ufem+(1-alpha)*ufem_prev;                                   % [Wb]   : relaxation of the solution
  % E.4. Check for convergence
  relerrnlin=norm(ufem-ufem_prev)/norm(ufem);                              % []     : relativ nonlinear error

...
for q=1:length(fld)
  switch fld{q}
    ...
    case 'node'
      ...
      numnode=size(femmdata.node,1); fprintf(fout,'%d\n',numnode);
      ...
      if strcmp(femmdata.Frequency,'0')    % static problem
        switch femmdata.Version
          case 2011
            tmp=[femmdata.node(:,1:2) real(femmdata.node(:,3))];
            fprintf(fout,'%13.6e %13.6e %e\n',transpose(tmp));
          case 2016
            tmp=[femmdata.node(:,1:2) real(femmdata.node(:,3)) -ones(numnode,1)];
            fprintf(fout,'%13.6e %13.6e %e %d\n',transpose(tmp));
          otherwise
        ...
        switch femmdata.Version
          case 2011
            ...
            fprintf(fout,'%13.6e %13.6e %e %e\n',transpose(tmp));
          case 2016
            tmp=[femmdata.node(:,1:2) real(femmdata.node(:,3)) imag(femmdata.node(:,3)) -ones(numnode,1)];
            fprintf(fout,'%13.6e %13.6e %e %e %d\n',transpose(tmp));
          otherwise
            error('unknown version');
        end
      end
    case 'elem'
      numelem=size(femmdata.elem,1); fprintf(fout,'%d\n',numelem);
      if strcmp(femmdata.Frequency,'0')    % static problem
        switch femmdata.Version
          case 2011
            fprintf(fout,'%d %d %d %d\n',transpose(femmdata.elem(:,1:4)));
          case 2016
            tmp=[femmdata.elem(:,1:4) ones(numelem,1)*[-1 -1 -1 0]];
        ...
        switch femmdata.Version
          case 2011
            fprintf(fout,'%d %d %d %d\n',transpose(femmdata.elem(:,1:4)));
          case 2016
            tmp=[femmdata.elem(:,1:4) ones(numelem,1)*[-1 -1 -1]];

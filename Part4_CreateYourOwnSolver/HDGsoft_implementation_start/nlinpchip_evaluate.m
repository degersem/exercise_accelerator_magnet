function [H,nu,nud,dnudB2,wmagn,wmagnco]=nlinpchip_evaluate(nlin,B)

% function [H,nu,nud,dnudB2]=NLINPCHIP_EVALUATE(nlin,B)
...
%
% see also NLINPCHIP_INITIALISE

% A. Initialisation
[num,dim]=size(B);                                    % number of material cells and dimension
[Bm,Bangle]=pyth(B);                                  % magnitude and angles
if any(Bm>nlin.Bmax);                                 % indices of the points in the extrapolated part of the characteristic
  warning('Evaluation of the BH-characteristic beyond the largest data point, dangerous!\n');
end
